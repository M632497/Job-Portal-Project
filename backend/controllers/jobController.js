   ...
   if (maxSalary) {
    ...
    if (query.$and.length === 0) {
     ...
     try {
        ...
        //Ass id saved and applicationstatus to each job
...
export const getJobsEmployer = async (req, res) => {
     try {
        ...
        //get all jobs
        const jobs = await Job.find({ company: userId }).populate("company", "name companyName companyLogo").sort({ createdAt: -1}).lean(); //lean makes jobs plain js object so we can add new fields
