...
const handleStatusChange = async (jobId) => {
  try {
    console.log(API_PATHS.JOBS.TOGGLE_CLOSE(jobId))
    const response = await axiosInstance.put(API_PATHS.JOBS.TOGGLE_CLOSE(jobId));
    getPostedJobs(true);
    toast.success("Job updated successfully!");
  } catch (error) {
    console.error("error toggling the job", error);
  }
};
      ...
      <div className='min-h-screen p-4 sm:p-6 lg:p-8'>
        <div className='max-w-7xl mx-auto'>
          ...
          <div className='bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl shadow-black/5 border border-white/20 p-6 mb-8'>
            ...
            <div className='my-4'>
              <p className='text-sm text-gray-400'>Showing {paginatedJobs.length} of {filteredAndSortedJobs.length}{" "}</p>
            </div>
            ...
            <div className='bg-white/80 backdrop-blur-sm rounded-2xl border border-white/20 overflow-hidden'>
              {filteredAndSortedJobs.length === 0 && !isLoading ? (
                <div className='text-center py-12'>
                  ...
                  <p className='text-gray-500'>Try adjusting your search ot filter criteria</p>
                </div>
              ) : (
                <div className='w-[75vw] md:w-full overflow-x-auto scrollbar-thumb-gray-300 scrollbar-track-gray-100'>
                  <table className='min-w-full divide-y divide-gray-200'>
                    <thead className='bg-gradient-to-r from-gray-50 to-gray-100/50'>
                      <tr>
                        <th className='px-6 py-6 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider cursor-pointer hover:bg-gray-100/60 transition-all duration-200 mi-w-[200px] sm:min-w-0' onClick={() => handleSort("title")}>
                        ...
                        <th className='px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider cursor-pointer hover:bg-gray-100/60 transition-all duration-200 min-w-[130px] sm:min-w-0' onClick={() => handleSort("applicants")}>
                          <div className='flex items-center space-x-1'>
                            <span>Applicants</span>
                            <SortIcon field="applicants" />
                          </div>
                        </th>
                        <th className='px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider min-w-[180px] sm:min-w-0'>Actions</th>
                      </tr>
                    </thead>
                    <tbody className='bg-white divide-y divide-gray-200'>
                      ...
                      ? Array.from({ length: 5 }).map((_, index) => (
                        ...
                        <tr key={job.id} className='hover:bg-blue-50/30 transition-all duration-200 border-b border-gray-100/60' >
                          ...
                          <td className='px-6 py-5 whitespace-nowrap min-w-[200px] sm:min-w-0'>
                            <span className={`inline-flex px-3 py-1.5 text-xs font-semibold rounded-full ${job.status === "Active" ? "bg-emerald-100 text-emerald-800 border border-emerald-200" : "bg-gray-100 text-gray-700 border border-gray-200"}`}>{job.status}</span>
                          </td>
                          ...
                          <td className='px-6 py-5 whitespace-nowrap text-sm font-medium min-w-[180px] sm:min-w-0'>
                            <div className='flex space-x-2'>
                              <button className='text-blue-600 hover:text-blue-800 p-2 rounded-lg hover:bg-blue-50 transition-colors duration-200 cursor-pointer' onClick={() => navigate("/post-job", { state: { jobId: job.id },})}>
                                <Edit className='w-4 h-4' />
                              </button>
                              {job.status === "Active" ? (
                                <button onClick={() => handleStatusChange(job.id)} className='flex items-center gap-2 text-xs text-orange-600 hover:text-orange-800 p-2 rounded-lg hover:bg-orange-50 transition-colors duration-200 cursor-pointer'>
                                  <X className='w-4 h-4' />
                                  <span className='hidden sm:inline'>Close</span>
                                </button>
                              ) : (
                                <button onClick={() => handleStatusChange(job.id)} className='flex items-center gap-2 text-xs text-green-600 hover:text-green-800 p-2 rounded-lg hover:bg-green-50 transition-colors duration-200 cursor-pointer'>
                                  <Plus className='w-4 h-4' />
                                  <span className='hidden sm:inline'>Activate</span>
                                </button>
                              )}
                              <button onClick={() => handleDeleteJob(job.id)} className='flex items-center gap-2 text-xs text-red-600 hover:text-red-800 p-2 rounded-lg hover:bg-red-50 transition-colors duration-200 cursor-pointer'>
