...
import { Plus, Briefcase, Users, Building2, TrendingUp, CheckCircle2, Icon, } from 'lucide-react';
          ...
          >
            <div className='space-y-3'>
              {dashboardData?.data?.recentApplications?.slice(0,3)?.map((data, index) => (
                <ApplicantDashboardCard key={index} applicant={data?.applicant || ""} position={data?.job?.title || ""} time={moment(data?.updatedAt).fromNow()}/>
              ))}
            </div>
          </Card>
        </div>
